...
void StartUpDisplay()
{
	...
	pos.rot.y = 0.0f;
	pos.rot.z = 0.0f;
	pos.trans.x = -30.0f * SCALE;
	pos.trans.y = -30.0f * SCALE;
	pos.trans.z = -30.0f;
...
void RenderCube(unsigned char x, unsigned char y, unsigned char z)
{
	...
	sceGumTranslate(&vec);
	sceGumScale(&scale);
	sceGumDrawArray(SCEGU_PRIM_TRIANGLES, SCEGU_TEXTURE_FLOAT|SCEGU_NORMAL_FLOAT|SCEGU_VERTEX_FLOAT, 36, 0, Cube);
	sceGumPopMatrix();
}

/**
  * Created: 24/02/2011
  * Renders to the screen some information to test that rendering works
  */
void TestRender()
{	
	unsigned int i;
	...
	//sceGuTexImage(0, 32, 32, 32, texture_data0);
